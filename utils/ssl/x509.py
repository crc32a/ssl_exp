...
def validity_generaltime_to_datetime(general_time):
    date_string = general_time.getComponent().asOctets()
...
def validity_utctime_to_datetime(utc_time):
    date_string = utc_time.getComponent().asOctets()
...
def get_subject_cn(x509_der):
    ...
    rdn_seq = subj_name.getComponent()
    for rnd in rdn_seq:
        for attr in rnd:
    ...
    for ext in exts:
        ...
        if ext_id == rfc2459.id_ce_subjectAltName:
            ...
            for general_name in general_names:
                if general_name.getName() == "dNSName":
                    name_type = general_name.getName()
                    dns_name = general_name.getComponent().asOctets()
